...
export default function CasaDaCultura2026() {
  ...
  const obterSaudacaoOficial = (oficina: string) => {
    ...
    if (o.includes("PIANO")) return `${base}Piano${final}que as melodias tornem seus dias mais leves.`;
    if (o.includes("VIOLINO")) return `${base}Violino${final}que a música siga afinando os dias.`;
    if (o.includes("CANTO") || o.includes("CORAL")) return `${base}Técnica Vocal e Coro${final}que sua voz continue ecoando incentivo.`;
  ...
  const salvarAlunoNoBanco = async (index: number) => {
    ...
    if (aluno.id) {
      ...
      fetchDadosGlobais(); // Atualiza dados para busca ativa
      return aluno.id;
    } else {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black text-2xl uppercase italic bg-white tracking-widest animate-pulse">CARREGANDO SISTEMA...</div>;
          ...
          {listaProfessores.map(p => {
            ...
            return (
              <button key={p} onClick={() => { setProfSel(isPiano ? "MICHEL" : p); setFiltroOficina(isPiano ? "PIANO" : ""); setTela('lista'); }} className="border-4 border-black bg-white p-8 flex flex-col items-center shadow-[6px_6px_0px_#000] hover:translate-y-[-2px] transition-all">
                ...
                <span className="text-[10px] text-blue-600 mt-2 font-bold">{totalAlunos} ALUNOS CADASTRADOS</span>
              </button>
            )
          })}
        </div>
  ...
  if (tela === 'busca') {
    const alunosFaltosos = todosAlunos.map(a => {
      ...
      .sort((a, b) => (a.turma?.oficina || "").localeCompare(b.turma?.oficina || "") || (a.turma?.horario || "").localeCompare(b.turma?.horario || ""));
    ...
    return (
      <div className="min-h-screen p-8 bg-white italic font-black uppercase">
        <nav className="no-print flex justify-between items-center mb-8 border-b-4 border-black pb-4">
           <button onClick={() => setTela('menu')} className="border-4 border-black px-4 py-2 hover:bg-black hover:text-white transition-all">← VOLTAR AO MENU</button>
        ...
        <table className="w-full border-collapse border-4 border-black">
          <thead>
            <tr className="bg-black text-white text-[10px]">
              <th className="p-3 text-left border-r border-white">CURSO / OFICINA</th>
              ...
              <th className="p-3 text-left bg-blue-900">TELEFONE DE CONTATO</th>
            </tr>
          </thead>
          <tbody className="text-xs">
            {alunosFaltosos.map((a, i) => (
              <tr key={i} className="border-b-2 border-black hover:bg-red-50">
                ...
                <td className="p-3 font-bold text-blue-800">{a.telefone || "NÃO CADASTRADO"}</td>
              </tr>
            ))}
          </tbody>
        </table>
            ...
            <div key={d}>
              ...
              <div className="space-y-4">
                {turmasDoProf.filter(t => String(t.dias).includes(String(d))).map(c => {
                   ...
                   const lim = obterLimiteOficina(c.oficina);
                   const corBorda = n > lim ? 'border-red-600' : n === lim ? 'border-yellow-500' : 'border-black';
                   return (
                    <div key={c.id} onClick={() => {setIdAtivo(c.id); setTela('chamada');}} className={`bg-white border-4 p-4 cursor-pointer shadow-[6px_6px_0px_#000] flex justify-between items-center transition-all ${corBorda}`}>
                      ...
                      <div className="text-right font-black italic"><span className="text-lg">{n} / {lim}</span></div>
                    </div>
                )})}
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen italic font-black uppercase bg-white">
      <style jsx global>{` @media print { .no-print { display: none !important; } .folha-container { border: none !important; box-shadow: none !important; max-width: 100% !important; padding: 10mm !important; } } `}</style>
      <nav className="no-print bg-white border-b-4 border-black p-4 flex justify-between items-center px-8 sticky top-0 z-50 shadow-md">
        <button onClick={()=>setTela('lista')} className="border-4 border-black px-4 py-2 hover:bg-black hover:text-white transition-all font-black italic">← VOLTAR</button>
        <div className="flex gap-4">
          <button onClick={() => setAlunosLocais([...alunosLocais, {nome:"", telefone:"", posicao:alunosLocais.length, id:null}])} className="bg-blue-600 text-white px-4 py-2 text-[10px] border-4 border-black shadow-[4px_4px_0px_#000]">ADICIONAR ALUNO +</button>
          <select value={mes} onChange={e => setMes(Number(e.target.value))} className="border-4 border-black p-1 text-xs font-black uppercase italic cursor-pointer">{mesesNomes.map((m,i)=><option key={i} value={i}>{m}</option>)}</select>
      ...
      <div className="folha-container max-w-[1300px] mx-auto p-10 mt-4 border-4 border-black bg-white mb-10">
        <header className="flex justify-between items-end mb-6 border-b-8 border-black pb-4">
          <div>
            ...
            <div className="flex gap-3 text-sm items-center">
                ...
                <span className="border-2 border-black px-3 py-0.5 font-black">{curso?.oficina}</span>
                <span className="font-bold underline">{curso?.horario}</span>
            </div>
          </div>
          <div className="text-right">
            <span className="text-5xl block leading-none font-black italic">{mesesNomes[mes]}</span>
            <span className="text-[9px] text-gray-500 font-bold tracking-widest uppercase">Casa da Cultura - Jardim Europa</span>
          </div>
        </header>
        <table className="w-full border-collapse border-4 border-black">
          <thead>
            <tr className="bg-gray-100 italic text-[10px]">
              ...
              <th className="border-2 border-black p-2 text-left w-32 no-print">WHATSAPP</th>
              {(() => {
              ...
              <th className="border-2 border-black w-12 no-print">FALTAS</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                <tr key={aluno.id || `temp-${i}`}>
                  ...
                  <td className="border-2 border-black px-2">
                    <input className={`w-full bg-transparent outline-none font-black text-xs uppercase italic ${f >= 3 ? 'text-red-600 underline' : ''}`} 
                           value={aluno.nome || ""} 
                           onChange={(e) => { const n = [...alunosLocais]; n[i].nome = e.target.value.toUpperCase(); setAlunosLocais(n); }} 
                           onBlur={() => salvarAlunoNoBanco(i)} placeholder="NOME DO ALUNO" />
                  </td>
                  <td className="border-2 border-black px-2 no-print">
                    <input className="w-full bg-transparent outline-none font-black text-[10px] text-blue-800 italic" 
                           ...
                           onBlur={() => salvarAlunoNoBanco(i)} placeholder="(00) 00000-0000" />
                  </td>
                  {(() => {
                    const diasAlvo = String(curso?.dias).includes('2') ? [2, 4] : [1, 3];
                    const datas = [];
                  ...
                  <td className="border-2 border-black text-center text-sm no-print font-black text-red-600 italic">{f}</td>
                </tr>
              )
            })}
          </tbody>
        ...
        <footer className="mt-10 flex justify-between items-center">
            <p className="text-[10px] max-w-[60%] border-l-4 border-black pl-4 font-black italic">{obterSaudacaoOficial(curso?.oficina || "")}</p>
            <div className="text-center font-black italic">
              <div className="w-64 border-b-2 border-black mb-1"></div>
              <p className="text-[9px]">ASSINATURA DO PROFESSOR(A): {curso?.professor}</p>
            </div>
        </footer>
      </div>
    </div>
