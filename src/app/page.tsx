...
export default function CasaDaCultura2026() {
  ...
  async function fetchTurmas() {
    ...
    // Busca contagem real de alunos por turma
  ...
  const salvarAlunoNoBanco = async (index: number) => {
    ...
    if (aluno.id) {
      ...
      if (novo && novo[0]) {
        ...
        setAlunosLocais(n);
        // Atualiza a contagem global imediatamente
  ...
  // --- RENDERS ---
  if (loading) return <div className="h-screen flex items-center justify-center font-black text-2xl italic uppercase">SINCRONIZANDO COM SUPABASE...</div>;
  ...
  if (tela === 'menu') return (
    <div className="min-h-screen p-8 bg-[#F8FAFC] italic font-black uppercase text-center">
      ...
      <div className="mb-12">
        <button onClick={() => { setTela('evasao'); fetchTurmas(); }} className="bg-red-600 text-white p-6 border-4 border-black shadow-[8px_8px_0px_#000] hover:translate-y-[-4px] transition-all text-xl w-full md:w-auto px-12">⚠️ CENTRAL DE EVASÃO</button>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
        {[...new Set(turmas.map(t => t.professor))].sort().map(p => (
          <button key={p} onClick={() => {setProfSel(p); setTela('lista');}} className="border-4 border-black bg-white p-8 text-sm flex flex-col items-center shadow-[6px_6px_0px_#000] hover:translate-y-[-2px] transition-all">
            {p}
            <span className="text-[10px] text-blue-600 mt-2 font-bold italic">
              {(turmas.filter(t => t.professor === p).reduce((acc, t) => acc + (contagemAlunos[t.id] || 0), 0))} ALUNOS TOTAL
            </span>
          </button>
        ))}
      </div>
  ...
  if (tela === 'lista') {
    ...
    return (
      <div className="min-h-screen p-8 max-w-6xl mx-auto italic font-black uppercase">
        <button onClick={() => setTela('menu')} className="text-xs mb-8 border-2 border-black px-2 py-1 font-bold bg-gray-50 italic">← VOLTAR</button>
                ...
                {turmasDoProf.filter(t => String(t.dias).includes(String(d))).map(c => (
                  <div key={c.id} onClick={() => {setIdAtivo(c.id); setTela('chamada');}} className="bg-white border-4 border-black p-4 cursor-pointer shadow-[6px_6px_0px_#000] flex justify-between items-center hover:translate-y-[-2px] transition-all">
                    ...
                    <div className="text-right font-black italic">
                      <span className="text-lg">{(contagemAlunos[c.id] || 0)}</span>
                      <p className="text-[8px] text-gray-400">ALUNOS ATIVOS</p>
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="min-h-screen italic font-black uppercase bg-white">
      ...
      <nav className="no-print bg-white border-b-4 border-black p-4 sticky top-0 z-50 flex justify-between items-center px-8 shadow-md">
        <button onClick={()=>{setTela('lista'); fetchTurmas();}} className="text-xs border-4 border-black px-4 py-2 font-bold bg-white italic">← VOLTAR</button>
      ...
      <div className="folha-container max-w-[1300px] mx-auto p-10 mt-6 border-4 border-black shadow-2xl bg-white mb-20">
        <header className="flex justify-between items-end mb-8 border-b-8 border-black pb-4 uppercase italic">
          ...
          <div className="text-right font-black italic uppercase"><span className="text-5xl block leading-none">{mesesNomes[mes]}</span><span className="text-[10px] text-gray-400 italic font-bold">CASA DA CULTURA 2026</span></div>
        </header>
