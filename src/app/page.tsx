...
export default function CasaDaCultura2026() {
  ...
  const salvarAlunoNoBanco = async (index: number, idReal: any) => {
    ...
    if (aluno?.nome?.trim() !== "") {
      const { data } = await supabase.from('alunos').upsert({ 
        ...
        telefone: aluno.telefone || "", // NOVO CAMPO
        posicao: index 
      }).select();
  ...
  return (
    <div className="min-h-screen italic font-black uppercase bg-white">
      ...
      <div className="folha-container max-w-[1200px] mx-auto bg-white p-10 mt-6 border-4 border-black">
        ...
        <table className="w-full border-collapse border-4 border-black">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="border-2 border-black p-2 text-left w-40 no-print">Contato ADM</th>
            ...
            {alunosLocais.map((aluno, i) => {
              ...
              return (
                <tr key={i} className="h-10">
                  ...
                  {/* COLUNA WHATSAPP EXCLUSIVA ADM (NO-PRINT) */}
                  <td className="border-2 border-black px-2 no-print">
                    <div className="flex items-center gap-2">
                      <input 
                        className="w-full bg-transparent outline-none text-[10px] font-bold text-blue-600"
