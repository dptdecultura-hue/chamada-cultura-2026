...
// ════════════════════════════════════════════════════════════
// SISTEMA DE LIMITES PADRÃO POR OFICINA (SEM TYPESCRIPT)
// ════════════════════════════════════════════════════════════
const LIMITES_OFICINAS = {
  'VIOLÃO': 15,
  'VIOLINO': 10,
  'VIOLA': 3,
  'VIOLONCELO': 3,
...
// ════════════════════════════════════════════════════════════
// FUNÇÃO PARA OBTER LIMITE (PERSONALIZADO OU PADRÃO)
// ════════════════════════════════════════════════════════════
function obterLimiteTurma(turma) {
...
// ════════════════════════════════════════════════════════════
function getStatusLotacao(alunos, limite) {
...
export default function ChamadaTrimestral() {
  ...
  const [mensagemTurma, setMensagemTurma] = useState(null)

  // ════════════════════════════════════════════════════════════
  // ESTADO PARA FILTRO DE PROFESSOR NO GERENCIAMENTO
  // ════════════════════════════════════════════════════════════
  const [filtroProfessorGerenciar, setFiltroProfessorGerenciar] = useState('')

  // ════════════════════════════════════════════════════════════
  // ESTADO PARA SUBSTITUIÇÃO EM MASSA DE PROFESSOR
  // ════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════
  // ESTADO PARA MODAL DE EXCLUSÃO DE ALUNO
  // ════════════════════════════════════════════════════════════
  ...
  function formatarDiasTexto(diasArr) {
  ...
  function abrirModalSubstituir() {
    if (!filtroProfessorGerenciar) {
      alert('Selecione um professor no filtro primeiro!');
      return;
    }
    setProfessorSubstituir(filtroProfessorGerenciar);
    setNovoProfessor('');
  ...
  // ════════════════════════════════════════════════════════════
  // TELA GERENCIAR TURMAS (COM FILTRO E SUBSTITUIÇÃO EM MASSA)
  // ════════════════════════════════════════════════════════════
  if (tela === 'gerenciar_turmas') {
    // Lista única de todos os professores para o filtro
    const todosProfessores = [...new Set(turmas.map(t => t.professor))].sort();
    
    // Aplicar filtro de professor
    const turmasFiltradas = filtroProfessorGerenciar
      ? turmas.filter(t => t.professor === filtroProfessorGerenciar)
      : turmas;

    // Agrupa turmas filtradas por unidade
    ...
    const totalFiltrado = turmasFiltradas.length;
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] p-8">
        <div className="max-w-7xl mx-auto">
          ...
          {/* ════════════════════════════════════════════════════════════ */}
          {/* FILTRO POR PROFESSOR + BOTÃO SUBSTITUIR */}
          {/* ════════════════════════════════════════════════════════════ */}
          <div className="bg-white border-4 border-black p-4 mb-6 shadow-[4px_4px_0px_#000]">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <select
                value={filtroProfessorGerenciar}
                onChange={e => setFiltroProfessorGerenciar(e.target.value)}
              ...
              {filtroProfessorGerenciar && (
                <>
                  <button
                    onClick={() => setFiltroProfessorGerenciar('')}
                  ...
                  {/* ════════════════════════════════════════════════════════ */}
                  {/* BOTÃO SUBSTITUIR PROFESSOR EM MASSA */}
                  {/* ════════════════════════════════════════════════════════ */}
                  <button
              ...
              <span className="text-xs text-gray-500 font-bold ml-auto">
                {totalFiltrado} turma{totalFiltrado !== 1 ? 's' : ''} encontrada{totalFiltrado !== 1 ? 's' : ''}
              </span>
            </div>
            ...
            {filtroProfessorGerenciar && (
              <div className="mt-2 text-[10px] text-gray-400">
                {totalFiltrado > 0 ? (
                  <span>👨‍🏫 Mostrando turmas de <strong>{filtroProfessorGerenciar}</strong></span>
                ) : (
                  <span>⚠️ Nenhuma turma encontrada para <strong>{filtroProfessorGerenciar}</strong></span>
                )}
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-[9px] text-gray-400 mt-1">
                    Deixe em branco ou 0 para usar o limite padrão da oficina
                  </p>
                </div>
              </div>
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className={`text-xs font-bold mt-1 ${status.cor}`}>
                              ...
                              <span className={`text-[10px] ml-1 font-bold ${status.cor}`}>
                                {status.label}
                              </span>
                            </p>
                          </div>
          ...
          {totalFiltrado === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 mt-2">Tente ajustar o filtro ou criar uma nova turma</p>
            </div>
          )}
        </div>
      </div>
              ...
              <div key={chave || `sem-dia-${i}`}>
                ...
                <div className="space-y-4">
                  {turmasGrupo.map(c => {
                    ...
                    const isPersonalizado = c.limite_personalizado && c.limite_personalizado > 0;

                    return (
                      <div 
                        key={c.id} 
                        onClick={() => { setIdAtivo(c.id); setTela('chamada'); }} 
                        className={`bg-white border-4 p-4 cursor-pointer shadow-[6px_6px_0px_#000] flex justify-between items-center hover:translate-y-[-2px] transition-all ${status.bg}`}
                      >
                        <div>
                          ...
                          {isPersonalizado && (
                            <span className="text-[8px] text-gray-400 font-bold ml-1">(limite personalizado)</span>
                          )}
                        </div>
                        <div className="text-right font-black italic">
                          <span className={`text-lg ${status.cor}`}>{status.icone} {n} / {limit}</span>
                          <span className="block text-[10px] text-gray-400">
                            {Math.round((n/limit)*100)}% - {status.label}
                          </span>
                        </div>
                      </div>
                    )
